...
void ar7240_set_sys_freq(void){

	u32 cfgpll;
	u16 *pll_offset;
	unsigned num;
	pll_offset=CFG_PLL_OFFSET;
	if(CFG_PLL_MAGIC==(*pll_offset)){
		pll_offset++;
		for(num=0;num<4;num++){
			if((*pll_offset)==0x0001){
				pll_offset++;
				cfgpll=*pll_offset;
				pll_offset++;
				cfgpll=(cfgpll<<16)|(*pll_offset);
				//printf("pllconfig=%x\n",cfgpll);
	...
	//printf("config=[0x%8x]\n",PLL_CONFIG_CPU_DDR_AHB);
	ar7240_reg_wr(AR7240_CPU_PLL_CONFIG,PLL_CONFIG_CPU_DDR_AHB);
	//udelay(100);
